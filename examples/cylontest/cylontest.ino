...
void cylon(uint8_t mode, uint16_t loopspeed, uint16_t numberofpasses,
  ...
  int32_t cylonWidth = invert ? kInvertedCylonWidth : kCylonWidth;
  ...
  while (!numberofpasses || pass <= (numberofpasses - 1)) {
    ...
    int16_t start_index =
        affects_whole_string ? 0 : max(0, leadpos - cylonWidth);
    int16_t end_index = affects_whole_string
                            ? strip.numPixels()
                            : min(strip.numPixels(), leadpos + cylonWidth + 1);
