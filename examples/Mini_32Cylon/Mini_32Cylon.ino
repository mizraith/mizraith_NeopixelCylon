...
void setCurrentMode() {
  ...
  temp = temp | (k << D10_shift);
  k = ~digitalRead(11);
  k = k & 0x01;    //only need last bit
  temp = temp | (k << D11_shift);
  k = ~digitalRead(12);
  k = k & 0x01;    //only need last bit
  temp = temp | (k << D12_shift);
  k = ~digitalRead(13);
  k = k & 0x01;    //only need last bit
  k = 0; //###############################HACK  this DIP ain't working
  temp = temp | (k << D13_shift);
  k = ~digitalRead(A0);
  k = k & 0x01;    //only need last bit
  temp = temp | (k << A0_shift);
  k = ~digitalRead(A1);
  k = k & 0x01;    //only need last bit
  temp = temp | (k << A1_shift);
  k = ~digitalRead(A2);
  k = k & 0x01;    //only need last bit
  temp = temp | (k << A2_shift);
  k = ~digitalRead(A3);
  k = k & 0x01;    //only need last bit
  temp = temp | (k << A3_shift);
...
void demoMode() {
  ...
  cylon(MODE_RED, 2000, 5);
  cylon(MODE_GREEN, 2000, 5);
  cylon(MODE_BLUE, 2000, 5);
  cylon(MODE_RAINBOW_NARROW, 2000, 5);
  cylon(MODE_RAINBOW_WIDE, 2000, 5);
  invertcylon(MODE_RED, 2000, 5);
  invertcylon(MODE_GREEN, 2000, 5);
  invertcylon(MODE_BLUE, 2000, 5);
  invertcylon(MODE_RAINBOW_NARROW, 2000, 5);
  invertcylon(MODE_RAINBOW_WIDE, 2000, 5);
}
...
// to 256 to allow for even transitions.
//
//
void cylon(uint8_t mode, uint16_t loopspeed, uint16_t numberofpasses) {
  ...
  if ( (mode == MODE_RAINBOW_WIDE) &&
       (MODE_DEMO != (CURRENT_MODE >> cylon_shift))  ) {
    numberofpasses = 256;
  }
